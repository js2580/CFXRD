    ...
    # for pointNo in [865, 872]:
        ...
        if cat[pointNo] == 'Fibre':
            ...
            if fit002 == True:
                ...
                metaData.ResultArray.loc[pointNo, 'chi_CF_002'] = FittingOutput.chisqr # (FittingOutput.chisqr, FittingOutput.redchi)
                # FWHM 
...
#%%            
            ...
            metaData.ResultArray.loc[pointNo, 'chi_CF_100'] = FittingOutput.chisqr # (FittingOutput.chisqr, FittingOutput.redchi)
            # FWHM 
            if spec["model"][0]["type"] == 'GaussianModel':    
                FWHM = 2 * FittingOutput.params['m0_sigma'].value * np.sqrt(2 * np.log(2)) # Gaussian
            elif spec["model"][0]["type"] == 'PseudoVoigtModel':
